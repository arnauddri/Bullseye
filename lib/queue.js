...
Queue.prototype.countJobs = function(callback){
  var _this = this;

  var queueTypes = ['wait', 'active', 'completed', 'failed'];
  var jobCount = {};

  async.mapSeries(queueTypes, function(queueType, done){
    var isList = (queueType === 'wait') ? true : false;

    _this.getJobs(queueType, isList).then(function (jobs) {
        jobCount[queueType] = (jobs) ? jobs.length : 0;
