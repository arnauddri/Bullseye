...
var states = [
  {key: 'wait', name: 'Queued', type: 'LIST', canRetry: false, canRemove: true},
  {key: 'active', name: 'Active', type: 'LIST', canRetry: true, canRemove: true},
  {key: 'failed', name: 'Failed', type: 'SET', canRetry: true, canRemove: true},
  {key: 'completed', name: 'Completed', type: 'SET', canRetry: false, canRemove: true}
];
...
exports.mount = function (app) {
  ...
  app.get('/queues/:name/:state?', function (req, res) {
    ...
    req.bull.getJobs(state.key, state.type, 0, limit).then(function (jobs) {
      jobs = jobs || [];

      // Sort jobs by id desc (recent first)
      jobs.reverse();

      var jobs = _(jobs).compact()
        .map(function (job) {
          var blob = job.toData();
  ...
  app.all('/queues/:name/jobs/:job_id*', function (req, res, next) {
    req.bull.getJob(req.params.job_id).then(function (job) {
      req.job = job;
      next();
    }).error(function (err) {
      res.send(500, err.message);
  ...
  app.post('/queues/:queue_name/jobs/:job_id/retry', function (req, res) {
    if (req.job && req.job.retry) {
      ...
      res.send(500, 'no retry on: ', req.job);
    }
  });

  app.delete('/queues/:queue_name/jobs/:job_id', function (req, res) {
    if (req.job && req.job.remove) {
      ...
      Queue.Job.removeFromId(req.bull, req.params.job_id).then(function () {
        res.send(200);
      }).error(function (err) {
        res.send(500, err.message);
      });
  ...
  app.post('/queues/:queue_name/retry_all/:state', function (req, res) {
    ...
    req.bull.getJobs(state.key, state.type).then(function (jobs) {
      if (jobs && jobs.length !== 0) {
        var jobRetries = jobs.map(function (job) {
          if (job)
            return job.retry();

          return Promise.resolve();
        });
        return Promise.all(jobRetries);
      }
    }).then(function () {
      res.send('OK');
    }).error(function (err) {
  ...
  app.post('/queues/:queue_name/remove_all/:state', function (req, res) {
    ...
    req.bull.getJobs(state.key, state.type).then(function (jobs) {
      if (jobs && jobs.length !== 0) {
        var jobRemovals = jobs.map(function (job) {
          if (job)
            return job.remove();

          return Promise.resolve();
        });
        return Promise.all(jobRemovals);
      }
